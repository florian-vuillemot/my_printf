...
** Last update Mon Nov 16 11:07:08 2015 Florian Vuillemot
*/
					...
					t_flag *flag, va_list *list)
{
  ...
  while (string && string->string && string->string[cur])
    if (string->string[cur++] == '%' && string->string[cur] &&
  	(nb_pop = there_are_dollar(&string, cur, flag, &pop_star)) > 0)
      {
  	i = cur;
  	while (pop_star && string->string[i] && string->string[i - 1] != '*')
  	  i++;
  	ins = get_insert_dollar(string->string + i, flag, nb_pop, list);
  	ins = replace_dollar_help_free(string, cur - 1, --i, ins);
  	if (pop_star)
  	  string = replace_star_dollar(string, cur, pop_star, list);
  	pop_star = 0;
      }
  return (string);
}
